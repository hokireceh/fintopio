...
class Fintopio {
  ...
  async sendToTelegram(message) {
    if (!message || typeof message !== 'string' || message.trim() === '') {
      console.log("Tidak ada pesan untuk dikirim ke Telegram.");
      return;
    }
    ...
    try {
      ...
      if (response.data.ok) {
        ...
        console.error(`Telegram API mengembalikan status error: ${response.data.description}`);
      }
    } catch (error) {
      if (error.response) {
        ...
        if (statusCode === 400) {
          ...
          console.error(`Detail: ${JSON.stringify(errorData)}`);
        } else if (statusCode === 401) {
          console.error(`Error 401: Unauthorized - ${errorMessage}`);
          console.error("Periksa token bot Anda dan pastikan token tersebut benar dan belum kadaluarsa.");
        } else {
          console.error(`Telegram API error: ${statusCode} - ${errorMessage}`);
          console.error(`Detail: ${JSON.stringify(errorData)}`);
        }
      } else if (error.request) {
        console.error("Tidak ada respons dari server Telegram.");
        console.error(`Permintaan yang dikirim: ${JSON.stringify(error.request)}`);
      } else {
        console.error(`Kesalahan dalam konfigurasi atau pengiriman: ${error.message}`);
      }
    }
  }
